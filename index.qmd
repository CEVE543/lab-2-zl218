...
Let's say we want to study rainfall at a particular point. I use NASA Johnson Space Center in Houston as our example:

```{julia}
target_location = "NASA Johnson Space Center"  # <1>
target_lon = -95.09333              # <2>
target_lat = 29.55301               # <3>
```

1. Descriptive name for our target location (change this for different places!)
2. NASA Johnson Space Center's longitude coordinate (negative means west of Greenwich)
3. NASA Johnson Space Center's latitude coordinate (positive means north of equator)
...
```{julia}
	# GEV parameters for NASA Johnson Space Center area (units: inches)
	# Try modifying these values to better fit your data!
	μ = 3.0    # Location parameter - shifts the distribution left/right
	σ = 1.8    # Scale parameter - controls spread (must be > 0)
	ξ = 0.2   # Shape parameter - controls tail behavior

	NASACenter_gev = GeneralizedExtremeValue(μ, σ, ξ)  # <1>
```
...
```{julia}
	...
	plot_return_periods(NASACenter_gev)
```
...
```{julia}
	...
	function plot_empirical_vs_theoretical(station_data, gev_dist, station_info)
		...
		emp_levels_in = (emp_levels isa AbstractVector{<:Unitful.AbstractQuantity}) ?
		ustrip.(uconvert.(u"inch", emp_levels)) : Float64.(emp_levels)
		scatter!(ax, emp_periods, emp_levels_in,   
	...
	plot_empirical_vs_theoretical(closest_rainfall, NASACenter_gev, closest_station)
```
...
*The closest station to NASA Johnson Space Center shows the trend in positive way. The station name is "CLEAR CK AT BAY AREA BLVD" and the station ID is 60-0011. The station is near the sea in the Houston area. Indicating the increase of rainfall patteren in Houston area. Although most of the years the rainfall stays between 5 - 10 inches, there were 5 years have the strong precipitaiton that between 10 inches within 1990 and 2020. The rain depth is increasing.*



**Station with apparent negative trend:**

*The station "QUANAH 2 SW" shows the trend in negative way. The station ID is 41-7336. The station, which is located near the Quanah Municipal Airport, near the state line with Oklahoma, shows the trend in decrease in annual precipitation depth. Though there were some intense precipitaion events, the average precipitation data remain around 2 inches. The precipitation data in recent years are lowere than the data 40 years ago. The negative trend is not clear but the precipitation is reducing.*
...
- Final: $\mu = 3.0$ $\sigma = 1.8$, $\xi = 0.2$

**Parameter Effects:**

**What happened when you increased/decreased $\mu$ (location parameter)?**

*With $\sigma$ and $\xi$ fixed, changing $\mu$ shifts the GEV curve vertically without altering its shape. When $\mu = 2.0$, the curve drops by about one inch (compared with my parameters), reducing overestimation and improving mid-range alignment but slightly underestimating at higher return periods. When $\mu = 4.0$, the curve rises by about one inch, which helps if red points lie above the $\mu = 3.0$ curve but otherwise creates overestimation, especially in the lower range. When $\mu = 6.0$, the curve moves up three inches, clearly overpredicting across all return periods. In practice, $\mu$ is best tuned to center the curve on the red points around 10–25 years, while $\sigma$ and $\xi$ handle slope and tail adjustments.*


**What happened when you increased/decreased $\sigma$ (scale parameter)?**

*With $\mu = 3.0$ and $\xi = 0.2$ fixed, adjusting $\sigma$ changes the steepness of the GEV curve and controls how quickly return levels rise with increasing return period. When $\sigma = 0.8$, the curve is flatter and compressed, meaning differences between low and high return periods are small, which can underestimate extremes relative to the red points. When $\sigma = 1.3$, the curve spreads out moderately, producing a reasonable rise in return levels that often balances mid- and high-period fit. When $\sigma = 2.0$, the curve becomes noticeably steeper, with return levels increasing rapidly at higher return periods, which can improve alignment if the red points at long return periods are high, but may overpredict in the lower range. Overall, increasing σ stretches the curve vertically and emphasizes differences across return periods, while decreasing $\sigma$ compresses it and diminishes the spread.*



**What happened when you changed $\xi$ (shape parameter)?** 

*With $\mu = 3.0$ and $\sigma = 1.8$ fixed, changing $\xi$ mainly affects the right tail of the GEV curve. When $\xi = - 0.1$, the curve has a light tail, so the high return period end flattens and predicted extremes grow slowly, often underestimating rare events. At $\xi = 0.0$ (the Gumbel case), the curve rises more steadily, producing a moderate tail that gives balanced growth in return levels. With $\xi = 0.15$, the curve bends upward more strongly, so higher return periods increase faster, matching data if observed extremes are large. When $\xi = 0.23$, the heavy tail exaggerates this effect, making the right side of the curve very steep and potentially overpredicting the most extreme return levels. In short, decreasing $\xi$ lightens the tail and suppresses extremes, while increasing $\xi$ makes the curve heavier and raises the high return period values.*
...
*The fitted GEV distribution provides a reasonable overall representation of the closest station’s rainfall extremes. In the mid-range return periods, roughly between 10 and 25 years, the red dots fall fairly close to the blue curve, showing that the model captures the central tendency of the data. This indicates that the chosen parameters align the distribution well with the bulk of observed extremes. At the lower end, around the 5-year return period, the red dots tend to fall slightly above the curve, suggesting the model underestimates smaller return levels. At the high end, beyond 50 years, the curve rises more steeply than the observed points, so the model may be overpredicting rare extremes compared to the available data. These deviations highlight that while the GEV gives a good baseline fit, especially in the middle range, adjustments in scale or shape could improve the representation of both the low and high tails.*



**Additional observations:**

*One additional observation is that the spread of the red points becomes wider at higher return periods, reflecting greater uncertainty in estimating rare events from limited data. This makes the tail of the GEV curve particularly sensitive to parameter choices, and small changes in σ or $\xi$ can noticeably alter the fit in this region.*
